    ...
    <Card className="p-6 border-white/5 bg-white/[0.02] backdrop-blur-xl">
      <div className="flex justify-between items-center mb-8 px-2">
        ...
        <div className="flex gap-2">
          <button onClick={() => changeMonth(-1)} className="p-2 bg-white/5 hover:bg-white/10 rounded-lg border border-white/10 transition-all text-slate-400 hover:text-accent-cyan">
            <ChevronLeft size={20}/>
          </button>
          <button onClick={() => changeMonth(1)} className="p-2 bg-white/5 hover:bg-white/10 rounded-lg border border-white/10 transition-all text-slate-400 hover:text-accent-cyan">
      ...
      <div className="grid grid-cols-7 gap-3">
        ...
        {Array(firstDay).fill(null).map((_, i) => (
          <div key={`pad-${i}`} className="h-32 bg-white/[0.01] rounded-2xl border border-white/[0.02]"></div>
        ))}
        ...
        {days.map(day => {
          ...
          return (
            <div key={day} className="h-32 bg-white/5 rounded-2xl p-3 border border-white/5 hover:border-accent-cyan/30 transition-all group relative overflow-hidden">
              <span className="text-xs font-black text-slate-500 group-hover:text-accent-cyan transition-colors">{day}</span>
              ...
              <div className="mt-2 space-y-1.5">
                ...
                {xdItems.map((item, i) => (
                  <div key={i} className="text-[9px] font-bold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 px-2 py-0.5 rounded-md flex justify-between">
                    <span className="truncate">{item.symbol.split('.')[0]}</span>
