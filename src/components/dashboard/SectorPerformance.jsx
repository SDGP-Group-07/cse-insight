...
const SectorPerformance = () => {
  ...
  const sectorsStatus = status.sectors;

  if (sectorsStatus.error) {
    return (
      <Card>
        <div className="flex flex-col gap-3">
          ...
          <Button
            ...
            onClick={() => refreshSection('sectors')}
          >
            Retry
          </Button>
        </div>
  ...
  if (sectorsStatus.loading && sectors.length === 0) {
    return (
      <Card>
        <p className="text-sm text-gray-300">Loading sector performance...</p>
      </Card>
    );
  }

  if (sectorsStatus.empty) {
    return (
      <Card>
        <div className="flex flex-col gap-3">
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-primary-dark border border-white/10 p-3 rounded-lg shadow-xl">
          ...
          <p
            className={`text-sm ${payload[0].value >= 0 ? 'text-accent-green' : 'text-red-500'}`}
          >
            {payload[0].value > 0 ? '+' : ''}
            {payload[0].value}%
          </p>
